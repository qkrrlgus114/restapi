...
public interface InquiryService {
    // 문의 등록하기
    void inquiryRegister(InquiryRequestDTO inquiryRequestDTO, Member member);

    // 내 모든 문의 가져오기
    InquiryListResponseDTO getMyInquiries(int page, Member member);

    // 특정 상세 문의 내역 가져오기
    InquiryInfoResponseDTO getTargetInquiry(Long inquiryId, Member member);
}
